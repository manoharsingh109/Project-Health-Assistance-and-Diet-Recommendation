...
st.sidebar.header("ℹ️ Your Information")
...
allergies=st.sidebar.selectbox("Allergies",["Allergies","Non-Allergies"])
##---------------------------------------##
...
if tab1:
    if st.button("Recommend Diet"):
        if client:
            try:
                db=load_rag()
                search_quary=f"""diet_type{diet_type}
                                  Healthy food
                                  protein
                                  Allergies{allergies}"""
                docs=db.similarity_search(search_quary,3)
...
if tab2:
    ...
    if st.button("Ask AI"):
        ...
        prompt=f"""You are an AI health and nutrition
                ...
                Use the following knowledge to answe
                the user's question.
